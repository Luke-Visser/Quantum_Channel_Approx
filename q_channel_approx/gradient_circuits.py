...
from q_channel_approx.training_data import TrainingData, measure_rhoss, wasserstein_all
...
def wass_loss_fac(circuit: PulseCircuit, training_data: TrainingData):
    """
    Creates loss function and gradient function for loss function
    Tr[sigma(rho_1 - rho_2)]^2
    ...
    def loss(theta, weights = None):
        U = circuit(theta)
        rhos = evolve_rho(U, training_data)[1:]
        meas_rhos = measure_rhoss(rhos, training_data.Os())
        
        if weights != None:
            error = np.sum(weights * (meas_rhos - training_data.Esss[1:]) )/(K*L*N)
        else:
            error, weigths = wasserstein_all(meas_rhos, training_data.Esss[1:], training_data.Os.Os_ids)
    ...
    def grad(theta):
        ...
        error, weights = wasserstein_all(meas_rhos[1:], training_data.Esss[1:], training_data.Os.Os_ids)
        ...
        for n1 in range(1, N+1): #repetitions on U
            for n2 in range(N+1-n1): #index for rho_n
                ...
                #matrices = np.einsum('aij, jk, bkl -> abil', paulis_Uext[n1-1], U, rhos_approx_ext[n2], optimize = 'greedy')
                #eta_T += np.einsum('kl, klij -> ij', traces[n1+n2], matrices, optimize= 'greedy' )
                eta_T += np.einsum('kl, klij -> ij', weights[n1+n2], matrices, optimize= 'greedy' )
...
def gradCircuit_fac(
    ...
    match loss_type:
        ...
        case "wasserstein":
            loss, grad = wass_loss_fac(circuit, training_data)
            return GradCircuit(**circuit._asdict(), loss = (loss, loss_l2), grad = (grad, grad_l2))
